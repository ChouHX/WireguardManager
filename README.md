...
开启「网关转发」后，该设备可作为网关，让其他设备经它访问 VPN。此处的**客户端网卡名**要填**该设备自己**用于 NAT 的物理网卡（如 `eth0`、`wlan0`），它会被写进客户端配置的 `PostUp`/`PreDown`：

```
PostUp = iptables -t nat -A POSTROUTING -o wlan0 -j MASQUERADE; ...
```

注意区分两个网卡：**客户端网卡**在此处按设备填写（服务器无法探测），**服务器出口网卡**在管理界面「系统设置 → 网络 → 出口网卡」中配置，界面会给出探测到的候选项。两者互不相干，填错会导致客户端连上后无法上网。
