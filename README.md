...
2. 探测无响应，但**最近 13 秒内收到过**对端流量（保活包即可）→ **在线**；
3. 两者都不满足 → 计一次失败，连续 2 次后判**离线**。

实测（真实 WireGuard 隧道，客户端保活 10 秒）：**关闭客户端后约 18 秒转为离线**。

判定耗时取决于第 2 步的流量窗口：它必须大于客户端保活间隔（默认 `PersistentKeepalive = 10` 秒），否则在线设备会在两次保活之间被误判。想更快就同时调小两者——例如保活 5 秒配 8 秒窗口，可在约 12 秒内判定，代价是保活流量与唤醒频率翻倍（约 550 KB/天）。

流量只统计**接收方向**（`rx`）：服务端每轮探测都会发出 TCP SYN，发送方向（`tx`）必然增长，若一并采信会让离线设备永远显示在线。

设备列表里会显示判定依据（探测有响应 / 隧道有流量 / 近期有流量 / 探测无响应），便于排查。
