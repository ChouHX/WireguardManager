...
//
// 取值依据：
//   - 探测间隔 1s：慢链路下单轮探测本身可能就要数秒，checking 标志会跳过重叠的
//     轮次，因此实际节奏由「探测耗时」而非这个间隔主导；
//   - 单次超时 3s：必须能容忍移动网络的往返延迟（常见数百毫秒，个别超过 1 秒）。
//     超时若小于链路 RTT，在线设备会被误判为无响应；
//   - 离线确认 2 次：连续两轮无响应且无流量才判离线，规避单次丢包造成的误报；
//   - 并发上限 16：限制同一时刻 fork 的探测数量。
//
// 综合起来：链路正常时设备掉线通常在数秒内被发现；慢链路或设备较多时，
// 判定会相应延后（每轮耗时接近超时值时，两轮确认即接近 6 秒）。
//
